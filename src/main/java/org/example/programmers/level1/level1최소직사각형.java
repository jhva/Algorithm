 ...
 */
public class level1최소직사각형 {


    public static void main(String[] args) {
        solve(new int[][]{{60, 50}, {30, 70},
                {60, 30}, {80, 40}});
    }

    static void solve(int[][] sizes) {
        int answer = 0;
        int max = 0;
        int min = 0;
        for (int i = 0; i < sizes.length; i++) {
            int maxi = Math.max(sizes[i][0], sizes[i][1]);
            int maxj = Math.min(sizes[i][0], sizes[i][1]);
//            System.out.println(maxi);
//            System.out.println(maxj);
            max = Math.max(max, maxi);
            min = Math.max(min, maxj);
